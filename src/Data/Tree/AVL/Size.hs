...
clipSize ASINT(c) t = let c_ = cSzh c t in if   isTrue# (c_ LTN L(0))
                                           then Nothing
...
cSzNP3 c t = if isTrue# (c LTN L(4)) then L(-1) else cSz2 DECINT2(c) t
cSzZ3  :: UINT -> AVL e -> AVL e -> UINT
cSzZ3  c l r = if isTrue# (c LTN L(5)) then L(-1)
                             else let c_ = cSz2 DECINT1(c) l
                                  in if isTrue# (c_ LTN L(2)) then L(-1)
                                                    else cSz2 c_ r
-- Parent Height (h) >= 4 !!
cSzNP,cSzZ :: UINT -> UINT -> AVL e -> AVL e -> UINT
cSzNP c h l r = if isTrue# (c LTN L(7)) then L(-1)
                              else let c_ = cSzG2 DECINT1(c) DECINT2(h) l       -- (h-2) >= 2
                                   in if isTrue# (c_ LTN L(4)) then L(-1)
                                                     else cSzG3 c_ DECINT1(h) r -- (h-1) >= 3
cSzZ c h l r = if isTrue# (c LTN L(9)) then L(-1)
