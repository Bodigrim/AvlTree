...
joinH' l hl r hr
                 = if isTrue# (hl LEQ hr) then let d = SUBINT(hr,hl) in joinHL d l r
...
spliceHL l hl b r hr = let d = SUBINT(hr,hl)
                       in if isTrue# (d EQL L(1)) then UBT2(N l b r, INCINT1(hr))
...
spliceHR l hl b r hr = let d = SUBINT(hl,hr)
                       in if isTrue# (d EQL L(1)) then UBT2(P l b r, INCINT1(hl))
