...
import Prelude ((+), (*), (-), odd, even, mod)
import Data.Bool (Bool(..), (&&), (||), not)
import Text.Read (read)
import Data.Function (($), (.), flip)
import Text.Show (show)
import Data.Eq ((==), (/=))
import Data.Foldable (length, all, and, any, or, concatMap, null, concat)
import Data.List ((++), take, zipWith, iterate, dropWhile, takeWhile, span, drop)
import Control.Monad ((>>))
import Data.Either (Either(..))
import Data.String (String)
import Data.Maybe (Maybe(..))
import System.IO (IO, putStrLn)
import Data.Ord (Ordering(..), compare, (>=), (<=), max, min, (<))
...
-- import Data.Tree.AVL.Internals.HAVL (HAVL(..), toHAVL, joinHAVL)
import Utils (exhaustiveTest, pathTree, allAVL, allNonEmptyAVL, checkHeight)
...
testAssertDelL =
 ...
 where test h _ t = let t_ = assertDelL t
                    in case checkHeight t_ of
                       Just h_ | (h_==h-1) || (h_==h)  -> asListL t_ == (drop 1 $ asListL t)
                       _                               -> False
...
testAssertDelR =
 ...
 where test h _ t = let t_ = assertDelR t
                    in case checkHeight t_ of
                       Just h_ | (h_==h-1) || (h_==h)  -> asListR t_ == (drop 1 $ asListR t)
                       _                               -> False
...
testRotateL = do title "rotateL"
              ...
              where test _ s t = all isOK rotations
                     where rotations = take s $ drop 1 $ iterate (map' (\n -> (n-1) `mod` s) . rotateL) t
